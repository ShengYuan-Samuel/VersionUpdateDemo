...
import com.nistart.versionupdatedemo.BuildConfig;
import com.nistart.versionupdatedemo.bean.Version;
import com.nistart.versionupdatedemo.download.DownloadHelper;
import com.nistart.versionupdatedemo.download.DownloadManager;
import com.nistart.versionupdatedemo.download.OnDownloadListener;
import com.nistart.versionupdatedemo.net.HttpUtils;
import com.nistart.versionupdatedemo.utils.AppUtils;
import com.nistart.versionupdatedemo.utils.SpUtils;
import com.nistart.versionupdatedemo.utils.StringUtils;
import com.nistart.versionupdatedemo.utils.ToastUtils;
...
public class UpdateManager {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_ON_FIND_NEW_VERSION:
                    ...
                    Version.DataBean.SoftwareBean software = version.getData().getSoftware();
                    mNewestVersionCode = software.getVersion();
                    mNewestVersionName = software.getVersionDesc();
                    mNewVersionContent = software.getContent();
                    updateUrl = software.getUpdateUrl();

                    if (mOnCheckUpdateListener != null)
                        mOnCheckUpdateListener.onFindNewVersion(mNewestVersionName,
                                mNewVersionContent,updateUrl);
                    break;
    ...
    public void checkUpdate(String apkInfoUrl, OnCheckUpdateListener onCheckUpdateListener) {
        ...
        HttpUtils.sendOkHttpRequest(apkInfoUrl, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (parseJson(strJson).getData().getSoftware().getVersion()> AppUtils.getAppVersionCode()) {
                    //最后一次缓存的时间超过缓存文件有效期，或者最后一次缓存的apk不是最新版本的apk，删除缓存apk
                    if ((System.currentTimeMillis() - mLastCacheSaveTime > getCacheSaveValidTime())
                            || (getCacheApkVersionCode() != parseJson(strJson).getData().getSoftware().getVersion())) {
                        clearCacheApkFile();
                        setCacheApkVersionCode(parseJson(strJson).getData().getSoftware().getVersion());
                    }
     ...
     * @return
     */
    private Version parseJson(String jsonData) {
        Gson gson = new Gson();
        Version version = gson.fromJson(jsonData, Version.class);
        return version;
    }
       private final String SP_KEY_CACHE_VALID_TIME = "sp_key_cache_valid_time";

       private  final String SP_KEY_CACHE_APK_VERSION_CODE = "sp_key_cache_apk_version_code";
}
